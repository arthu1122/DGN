...
class HeteroGNN(torch.nn.Module):
    def __init__(self, num_features_cell=890, hidden_channels=768, out_channels=128, num_layers=1, dropout=0.2):
        super().__init__()
        ...
        for _ in range(num_layers):
            conv = HeteroConv({
                ...
                ('drug', 'd-t', 'target'): GATConv((-1, -1), hidden_channels),
        ...
        self.reduction2 = nn.Sequential(
            nn.Linear(hidden_channels, 2048),
            nn.ReLU(),
            nn.Dropout(dropout),
            # nn.Linear(2048, 512),
            # nn.ReLU(),
    ...
    def forward(self, x_dict, edge_index_dict, drug1_id, drug2_id, cell_features):
